...
namespace nc
{
	...
	void LightComponent::SetProgram(const res_t<Program> program, const std::string& name)
	{
		...
		if (castShadow)
		{
			...
			// shadow view projection = shadowVP
	...
	{
		//orthographic light for now
		glm::mat4 projection = glm::ortho(-shadowSize * 0.05f, shadowSize * 0.05f, -shadowSize * 0.05f, shadowSize * 0.05f, 0.1f, 50.0f); 
